...
import './bootstrap';
//
window._ = require('lodash'); //utilities
require("parsleyjs"); //validation form's
import LazyLoad from "vanilla-lazyload"; //image's loader
window.$ = window.jQuery  = jQuery; //dom manipulation

var Sticky = require('sticky-js');

//#region Plugins-Inline
    /**
     * Owl Carousel v2.3.4
...
//#region UTILS
    //#region resizeWindow
         var hk_resizeWindowFn = new Array();
         var resizeWindow_callback = function (functions) {
         ...
         window.resizeWindow = function () {
             ...
             if (hk.window.innerWidth == 0) { hk.window.innerWidth = b }
             
             if (hk.window.innerWidth <= b) {
                 a = "xl"; if (b < 1200){ a = "lg" } if (b < 992){ a = "md" } if (b < 768){ a = "sm" } if (b < 576){ a = "xs" } 
             } else {
                 if (b > 0){ a = "xs" } if (b >= 576){ a = "sm" } if (b >= 768){ a = "md" } if (b >= 992){ a = "lg" } if (b >= 1200) { a = "xl" }
             }
             
             hk.window.innerWidth = b; hk.window.innerHeight = c;
             if (hk.screen.type != a) {
             }
             hk.screen.type = a;
             var $Html = jQuery(html);
             $Html.removeClass('screen-xs').removeClass('screen-sm').removeClass('screen-md').removeClass('screen-lg').removeClass('screen-xl');
             $Html.addClass('screen-'+hk.screen.type);
             resizeWindow_callback(hk_resizeWindowFn);
         };
    //#endregion

    //#region ScrollWindow
    var hk_scrollWindowFn = new Array();
    var scrollWindow_callback = function (functions) {
        if (functions.length > 0) {
            for (var i in functions) {
                eval(functions[i])
            }
    ...
    window.scrollWindow = function () {
        hk.window.scrollTop = $(window).scrollTop();
        scrollWindow_callback(hk_scrollWindowFn);
    };
    //#endregion ScrollWindow

    //#region Browser's
    var hk_browsers = function(){
    ...
    const hk_frm_basic = function (formID) {
        ...
        const form_submit = "#hk-btn-submit";
        ...
        //$('input', form).val('abe@abe.com'); $('input#phone', form).val('33'); $('textarea', form).val('abe');
    
        //region Hover effect
        const frm = $(formID);
        let frmFocus = false;
        let frmOver = false;
        $(".frm-event", frm).focus(function () {
            if (frmFocus == false) {
                frm.addClass('frm-focus');
                frm.find('.parsley-errors-list').show();
                frmFocus = true;
            }
        });
        $(frm).mouseenter(function () {
            frmOver = true;
        });
        //endregion
    
        //region validate parsley
        const floatingLabel = function (onload) { //console.log(onload);
            if (onload) {
                $.each($('input', '.bt-flabels__wrapper'), function (index, value) {
        ...
        const floatingLabelActions = function () {
            const $input = $(this);
            setTimeout((function () {
                if ($input.val()) {
        ...
        $('.bt-flabels__wrapper input').keydown(floatingLabelActions);
        $('.bt-flabels__wrapper input').change(floatingLabelActions);
    
        $('.bt-flabels__wrapper textarea').keydown(floatingLabelActions);
        $('.bt-flabels__wrapper textarea').change(floatingLabelActions);
    
        window.addEventListener('load', floatingLabel(true), false);
    
        const jsParsley = form.parsley();
        ...
        const resetForm = function () {
            ...
            $('input.checkbox-input', form).prop("checked", false);
            $('input', form).val('').removeAttr('disabled');
            $('textarea', form).val('').removeAttr('disabled');
            $('.bt-flabel__float', form).removeClass('bt-flabel__float');
        };
        //endregion validate parsley
    };
    //endregion frm_basic

    //#region => StickyMenu
    var stickyMenu = function(){ //console.log('stickyMenu');
        return false;
        if( hk.screen.type == 'xs' || hk.screen.type == 'sm' || hk.screen.type == "md" ){
            return false;
        }
        var menuPrimary = $('#menu-primary');
        var ele = $('#menu-sticky');
        //var target = $('.menu-primary', ele);
        var targetHeight = menuPrimary.height();
        var yposTop = 370;

        if( hk.window.scrollTop <= 300 ){
            menuPrimary.removeClass('menu-sticky');
        }
        if( hk.window.scrollTop > 300 ){
            menuPrimary.addClass('menu-sticky');
        }

        if( hk.window.scrollTop < yposTop ){
            menuPrimary.removeClass('sticky-active');
            //target.removeClass("animeFx");
        }

        if( hk.window.scrollTop > yposTop ){
            menuPrimary.addClass('sticky-active').removeClass('d-none');
        }

    };
    //#endregion => StickyMenu

    //#region => Instant Chat
    var instantChat = function(){
        var ele = $('#instant-chat');
        var offset = ele.offset();
        var eleHeight = 64;
        var eleFoot = $('.footer');
        var offsetFoot = eleFoot.offset();
        var footPosY = offsetFoot.top - (eleFoot.outerHeight()*2) - ( eleHeight );

        var yposTop = hk.window.innerHeight;
        var yposBottom = offsetFoot.top;
        var limitTop = 300 - eleHeight; //(offset.top - 780);

        //console.log("instantChat: "+ hk.window.scrollTop  + " > " +  limitTop + " height:" + eleHeight );
        if(  hk.window.scrollTop < limitTop ){
            ele.css({'top': (hk.window.innerHeight-42) +'px', 'bottom': 'auto', 'position' : 'fixed' });
        }

        if( hk.window.scrollTop > limitTop ){
            ele.css({'top':'auto', 'bottom': '42px', 'position' : 'fixed' });
        }

        if(  hk.window.scrollTop > footPosY ){
            ele.css({'top': yposBottom, 'bottom':'auto', 'position' : 'absolute' });
        }

    };
    //#endregion Instant Chat

    //-> Iniciamos funciones | example: <script>gFuncInit.push('test()');</script>
	;function InitFunctions( functions ){ if(functions.length > 0 ){ for(var i in functions ){ eval(functions[i]); } } }
 ...
 */
//#region PROYECT
    var hk_Navbar_active = function($xAnclaNow){
        window.location.hash = '#!'+$xAnclaNow;
        $('[hk-ancla-from="'+$.xAnclaAct+'"]').removeClass("active");
        $('[hk-ancla-from="'+$xAnclaNow+'"]').addClass("active");
        $.xAnclaAct = $xAnclaNow;
    }
    var hk_navbar_scroll = function(){
        var $xAnclaNow = "";
        //console.log( $.xAnclasTo.length );
        $.xAnclasTo.each( function(i,e){
            var $xPosY = ($(this).position()).top;
            var $xLast = $.xAnclasTo.length-1;
            var $xPosYend = $xPosY - $.Header.height() - 6;
            if( hk.window.scrollTop > $xPosYend ){
                $xAnclaNow = $(this).attr("hk-ancla-to") ? $(this).attr("hk-ancla-to") : "";
            }
        });
        if($xAnclaNow != $.xAnclaAct){
            hk_Navbar_active($xAnclaNow);
        }
    }
    var hk_page_home = function(){
        ...
        if ($("#contact-form").length == 1) {
            hk_frm_basic("#contact-form");
        };

        if($.xAnclasFrom.length>0){
            $.xAnclasFrom.each( function( index, element ){
                // OnClic
                $(this).click( function(e){ e.preventDefault();
                    var targetOffset = 0;
                    var distance = 0;
                    var animationTime = 0;
                    var $attr = $(this).attr("hk-ancla-from");
                    var $target = $('[hk-ancla-to="'+$attr+'"]');
                    // Establecer un tiempo mínimo y máximo para la animación
                    var minTime = 0; // Tiempo mínimo en milisegundos
                    var maxTime = 500; // Tiempo máximo en milisegundos
                    if($target.length>0){
                        targetOffset = ($attr !== 'inicio') ? ($($target).offset().top - $.Header.height()) : 0;
                        distance = Math.abs(targetOffset - $(window).scrollTop());
                        animationTime = Math.max(minTime, Math.min(maxTime, distance));
                        $('html,body').animate({scrollTop: targetOffset}, animationTime, "linear");
                        hk_Navbar_active($attr);
                    }
                });
            });
        };
    };
    var hk_page_contact = function(){
        if ($("#contact-form").length == 1) {
            hk_frm_basic("#contact-form");
        };
    };
...
(function($) {
    ...
    $.winScrollTop = 0;
    $.Header = $("#header");
    $.HeaderPosY = ($.Header.position()).top + ($.Header.height());

    $.xAnclaAct = 'inicio';
    $.xAnclasFrom = $('[hk-ancla-from]');
    $.xAnclasTo = $('[hk-ancla-to]');
    ...
    //#region resizeWindow
        resizeWindow(); //establece valores globales
        $window.resize(function(){ resizeWindow(); });
        //hk_resizeWindowFn.push('instantChat()');
    //#endregion resizeWindow

    //#region scrollWindow
    scrollWindow();
    $window.scroll(function(){ scrollWindow(); });
    hk_scrollWindowFn.push('instantChat()');
    hk_scrollWindowFn.push('hk_navbar_scroll()');
    hk_navbar_scroll();
    //#endregion scrollWindow
    ...
    $(document).ready(function () {
        ...
        if(gFuncInit.length>0){InitFunctions( gFuncInit );}
        ...
        if (isProduction == false) {
            ...
            if($_GET['grid']==1){
                $('body').addClass('grid-debug').append('<div class="grid"><div class="container"><div class="row"></div></div></div>');
            }
        }
